...
class Bppsimulator():
    def __init__(self, align, tree, config, output, tools, ext_length=None, root_length=None, gaps=False, mapping=None):
        ...
        bppsuite_path = os.path.join(home, "devel/bpp/bppsuite/bppSuite")
        bpp_phyl_path = os.path.join(home, "devel/bpp/bpp-phyl/build/src")
        bpp_seq_path = os.path.join(home, "devel/bpp/bpp-seq/build/src")
        bpp_core_path = os.path.join(home, "devel/bpp/bpp-core/build/src")
        # Add the bppsuite binary directory to PATH so that bppseqgen and other tools can be found
        os.environ["PATH"] = f"{bppsuite_path}:" + os.environ.get("PATH", "")
        # Add Bio++ library directories to LD_LIBRARY_PATH for dynamic linking
    ...
    def simulate(self):
        ...
        date = datetime.now()
        # Get the result type (e.g., BPP) and the main name (e.g., WAG_frequencies_posterior_extra_length_data2)
        result_type = self.outputsim.parent.name  # e.g., BPP
        main_name = self.outputsim.name  # e.g., WAG_frequencies_posterior_extra_length_data2
        # Simplify the main name for use in config directory naming
        parts = main_name.split('_')
        if len(parts) >= 4:
            short_main = f"{parts[0]}_{''.join([p[0].upper() for p in parts[1:4]])}"  # e.g., WAG_FPEL
            if len(parts) > 4:
                short_main += '_' + '_'.join(parts[4:])
        else:
            # Fallback: replace keywords with abbreviations
        ...
        for align_name in align_names:
            famname = align_name.split('.')[0]
            # Choose the correct tree path depending on whether ext_length or root_length is set
            ...
            # Replace placeholders in the config template with actual paths and values
        ...
        # Do not remove the temporary config directory (keep the config file)
        # shutil.rmtree(temp_config_rep)(keep the config file)
