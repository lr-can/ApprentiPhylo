...
class Computing_trees:
    ...
    def compute_tree(self, inputname, outputname):
        ...
        if results.returncode == 0:
            # écrire l'arbre dans un fichier .nw
            arbre_fichier = self.output / outputname
            with open(arbre_fichier, 'w') as f:
                f.write(results.stdout)

            # --- Lecture et affichage ---
            #arbre = Phylo.read(str(arbre_fichier), "newick")

            # Version linéaire ASCII
            #print(f"\nASCII tree for {outputname}:")
            #Phylo.draw_ascii(arbre)

            # Version graphique matplotlib
            #plt.figure(figsize=(8, 6))
            #Phylo.draw(arbre)
            #plt.title(f"Tree: {outputname}")
            #plt.show()

        else:
            print(f"Error computing tree for: {self.input / inputname}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--only', default=None,
                        help='path to a file listing the names of specific alignment files to be processed (one per line). If the option is supplied, only these files will be taken into account.')
